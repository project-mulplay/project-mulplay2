...
## 🖥️ 프로젝트 소개
MULPLAY는 개발자를 위한 크라우드 펀딩 사이트입니다.
좋은 아이디어가 있더라도 개인이나 소규모 개발팀은 자본에 한계가 있다는 것과
다양한 it 서비스를 접할 수 있는 플랫폼이 있었으면 좋겠다는 생각에 주제를 선정했습니다.
<br>
MULPLAY의 핵심가치는 프로젝트가 개발되기 전 미리 소비동향을 파악할 수 있을 뿐 아니라 
사용자들과의 상호작용을 통해 미리 유저베이스를 형성하고 
이를 통해 프로젝트의 성공적인 출시로 이어지도록 하는 것 입니다. 
...
### 🧑‍🤝‍🧑 맴버구성
 ...
 - 팀원1 : 김소정 - 회원 Cookie Token설정, recoil 설정, 라우터 구성, 마이 페이지, 리스트 페이지, 레이아웃 컴포넌트, 회원 탈퇴기능, 깃 규칙 & 전략설정, server 아키텍쳐 구성, DB설계 및 연동, API 구성, 백엔드 서버 배포, 
